...
public class CieloPaymentService : FluencyHub.PaymentProcessing.Application.Common.Interfaces.IPaymentService
{
    ...
    public async Task<FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult> ProcessPaymentAsync(
        Guid studentId,
        Guid enrollmentId,
        decimal amount,
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (status == 2 || status == 1)
                {
                    ...
                    return FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult.Success(transactionId);
                }
                else
                {
                    ...
                    return FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult.Failure(message);
                }
            }
            else
            {
                ...
                return FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult.Failure($"Erro no gateway de pagamento: {response.StatusCode}");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Exception occurred while processing payment for enrollment {EnrollmentId}",
                enrollmentId);
            return FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult.Failure($"Erro no processamento do pagamento: {ex.Message}");
        }
    }

    public async Task<FluencyHub.PaymentProcessing.Application.Common.Models.PaymentStatusResult> GetPaymentStatusAsync(string transactionId)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return FluencyHub.PaymentProcessing.Application.Common.Models.PaymentStatusResult.Create(transactionId, paymentStatus, message);
            }
            else
            {
                var errorContent = await response.Content.ReadAsStringAsync();
                _logger.LogError("Payment gateway returned error when checking status: {StatusCode}, {Error}",
                    response.StatusCode, errorContent);
                return FluencyHub.PaymentProcessing.Application.Common.Models.PaymentStatusResult.Create(transactionId, FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Desconhecido,
    ...
    public async Task<FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult> RequestRefundAsync(string transactionId, decimal amount)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (status == 10 || status == 11)
                {
                    ...
                    return FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult.Success(transactionId, refundTransactionId, amount);
                }
                else
                {
                    ...
                    return FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult.Failure(transactionId, message);
                }
            }
            else
            {
                ...
                _logger.LogError("Payment gateway returned error when requesting refund: {StatusCode}, {Error}",
                    response.StatusCode, errorContent);
                return FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult.Failure(transactionId, $"Erro na solicitação de reembolso: {response.StatusCode}");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Exception occurred while requesting refund for transaction {TransactionId}",
                transactionId);
            return FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult.Failure(transactionId, $"Erro no processamento do reembolso: {ex.Message}");
        }
    }

    private FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento MapCieloStatusToPaymentStatus(int cieloStatus)
    {
        return cieloStatus switch
        {
            1 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Pendente,
            2 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Autorizado,
            3 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Cancelado,
            4 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Concluido,
            10 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Reembolsado,
            11 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Reembolsado,
            12 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Pendente,
            13 => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Falha,
            _ => FluencyHub.PaymentProcessing.Application.Common.Models.StatusPagamento.Desconhecido
        };
    }

    private string GetCardNumberFromMasked(string maskedNumber)
    {
        // Em um cenário real, isso seria adequadamente tratado para segurança
        if (maskedNumber.Contains("4111"))
            return "4111111111111111";
        
        if (maskedNumber.Contains("5555"))
            return "5555555555554444";
        
        return "4111111111111111"; // Cartão padrão para testes
    }
} 