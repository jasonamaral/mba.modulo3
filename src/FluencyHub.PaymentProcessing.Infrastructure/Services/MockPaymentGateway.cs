...
public class MockPaymentGateway : IPaymentGateway
{
    ...
    public Task<FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult> ProcessPaymentAsync(string orderId, decimal amount, CardDetails cardDetails)
    {
        ...
        if (cardDetails.MaskedCardNumber.Contains("4111") || cardDetails.MaskedCardNumber.Contains("1111"))
        {
            ...
            return Task.FromResult(FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult.Success(transactionId));
        }
        else
        {
            _logger.LogWarning("MockPaymentGateway: Payment failed, invalid card number");
            return Task.FromResult(FluencyHub.PaymentProcessing.Application.Common.Models.PaymentResult.Failure("Card was declined"));
        }
    }
    ...
    public Task<FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult> ProcessRefundAsync(string transactionId, decimal amount, string reason)
    {
        ...
        if (_transactions.ContainsKey(transactionId))
        {
            ...
            return Task.FromResult(FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult.Success(
                transactionId, 
                refundTransactionId, 
                amount));
        }
        else
        {
            ...
            return Task.FromResult(FluencyHub.PaymentProcessing.Application.Common.Models.RefundResult.Failure(
                transactionId, 
